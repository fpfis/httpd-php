...
ENV TIMEOUT=120 DOCUMENT_ROOT="/var/www/html" PORT=8080 APACHE_EXTRA_CONF_DIR="" APACHE_ERROR_LOG="/var/log/apache.err" APACHE_ACCESS_LOG="/var/log/access" ALLOWOVERRIDE="none"
ENV PHP_VERSION="7.2" FPM_MAX_CHILDREN=100 FPM_START_SERVERS=20 FPM_MIN_SPARE_SERVER=10 FPM_MAX_SPARE_SERVER=30 FPM_MAX_REQUESTS=500 PHP_UPLOAD_MAX_FILESIZE="200M" PHP_POST_MAX_SIZE="220M" PHP_SESSION_PATH="/tmp" PHP_ERROR_LOG="/var/log/php.err" PHP_DISPLAY_ERRORS="Off" PHP_DEPENDENCIES="common cli fpm soap bz2 opcache zip xsl intl imap mbstring ldap mysql gd memcached redis curl sqlite bcmath"
...
RUN apt-get update && apt-get install unzip cronolog tzdata ssmtp git curl wget supervisor rsync -y && sed -ri -e 's@^mailhub=mail$@mailhub=127.0.0.1@' -e 's@^#FromLineOverride@FromLineOverride@' /etc/ssmtp/ssmtp.conf && ln -fs /usr/share/zoneinfo/Europe/Brussels /etc/localtime && dpkg-reconfigure --frontend noninteractive tzdata && apt-cache madison php | grep -q "1:${PHP_VERSION}+" && apt-get install apache2 php${PHP_VERSION} -y && apt-get install `for PHP_DEPENDENCY in ${PHP_DEPENDENCIES}; do echo -n "php${PHP_VERSION}-${PHP_DEPENDENCY} "; done` -y; phpdismod exif readline shmop sysvmsg sysvsem sysvshm wddx; apt-get clean all; rm -rf /var/lib/apt/lists/* /etc/apache2/mods-enabled/status.conf; chmod o+rx /etc/ssmtp; chmod o+r /etc/ssmtp/*; > /var/www/html/index.html
